...
export default function ConsultaRapida({ session, perfil, visitante, onLogout }) {
  ...
  const resultadoPendiente = useRef(null)

  // Solo visitantes (sin sesión y sin rol) ven el modal
  const debeVerModal = visitante && !session
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      // Mostrar modal solo si es visitante puro y aún no lo vio
      if (debeVerModal && !modalYaMostrado.current) {
        resultadoPendiente.current = data
        setStatus('idle')
        setShowModal(true)
      } else {
        setResult(data)
  ...
  function handleModalDone() {
    modalYaMostrado.current = true
    setShowModal(false)
    if (resultadoPendiente.current) {
      setResult(resultadoPendiente.current)
      setStatus('success')
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column' }}>
      ...
      {showModal && (
        <ModalVisitante
          onConfirmar={handleModalDone}
          onSaltar={handleModalDone}
        />
      )}
      ...
      <main style={{ flex:1, maxWidth:'960px', width:'100%', margin:'0 auto', padding:'2.5rem 1.5rem' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns: result ? '1fr 1fr' : '1fr', gap:'1.5rem', alignItems:'start' }}>
          <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'10px', padding:'1.5rem', boxShadow:'var(--shadow)' }}>
            <p style={{ fontSize:'11px', fontWeight:500, letterSpacing:'0.06em', textTransform:'uppercase', color:'var(--muted)', marginBottom:'1rem' }}>
              Imagen de entrada
            </p>
            ...
            {(status === 'error' || status === 'offline') && (
              <div style={{
                ...
                background: status === 'offline' ? 'var(--warn-lt)' : 'var(--danger-lt)',
                borderRadius:'10px', fontSize:'13px',
                color: status === 'offline' ? 'var(--warn)' : 'var(--danger)',
              }}>
                {error}
              </div>
            )}
